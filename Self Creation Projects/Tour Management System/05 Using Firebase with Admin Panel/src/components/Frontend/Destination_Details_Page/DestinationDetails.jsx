...
export default function DestinationDetails() {
    ...
    return (
        <Box>
            <DestinationDetailsHeader destination={destinations} />
            <Box id="destinationDetailsPage">
                <Flex className="inner-area" style={themeCss} justifyContent='space-between'>
                    <DestinationDescription destination={destinations} />
                    <Box className='inner-box-1'>
                        <InquiryForm />
                        {!initialLoad && <DestinationList excludeName={excludeName} destination={destinations} />}
                    </Box>
                </Flex>
            </Box>
        </Box>
